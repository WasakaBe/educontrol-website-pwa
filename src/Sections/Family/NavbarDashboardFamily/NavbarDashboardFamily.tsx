  ...
  return (
    <nav className="navbar-admin">
      ...
      <ul className={`navbar-admin-menu ${isMenuOpen ? 'active' : ''}`}>
        <li>
          <a href="#" onClick={() => onButtonClick('dashboardfamiliar')}>
            INICIO
          </a>
        </li>
        <li>
          <a href="#" onClick={() => onButtonClick('infoalumnofamiliar')}>INFORMACIÓN</a>
      ...
      <div className="navbar-admin-user">
        {user && user.foto_usuario ? (
          ...
          <span onClick={() => onButtonClick('perfilfamiliar')} className="user-name">{user.nombre_usuario}</span>
        )}
      </div>
    </nav>
  )
